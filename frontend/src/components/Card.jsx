...
export const Card = ({ id, title, author, url_cover, url_audio, categories, disabled }) => {
  ...
  const handleFavorite = (e) => {
    ...
    setIsFavorite(isFavorite => !isFavorite) // Alterna el estado de favorito
    setIconHeart(iconHeart => !iconHeart)
  }

  return (
    <Link to={`/book/${id}`}>
      <div
        className={`group ${disabled && 'cursor-auto'} relative group`}
      >
        <div className='aspect-square overflow-hidden relative'>
          ...
          {!disabled && (
            <button
              ...
              className={`absolute bottom-2 left-2 rounded-md p-2 transition-colors duration-300 hidden group-hover:block
                          ${iconPlay ? 'bg-htc-white text-black' : 'bg-htc-lightblue text-white'}
              `}
              style={{ width: '40px', height: '40px' }}
            >
              <div className="flex items-center justify-center h-full">
                <i className={iconPlay ? `bi bi-play text-2xl` : `bi bi-play-fill text-2xl`}></i>
              </div>
            </button>
          )}
          ...
          {!disabled && <button
            ...
            className={`absolute bottom-2 right-2 rounded-md p-2 transition-colors duration-300 hidden group-hover:block
              ${iconHeart ? 'bg-htc-white text-black ' : 'bg-htc-lightblue text-white'
              }`}
            style={{ width: '40px', height: '40px' }}
          >
            <i className={iconHeart ? `bi bi-heart text-lg` : `bi bi-heart-fill text-lg`}></i>
          </button>}
        </div>
